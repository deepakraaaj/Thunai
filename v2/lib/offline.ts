...
// en + ta required; tanglish carries the romanized Tamil variant. hi/hinglish
// fall back to en — an honest, pre-written safety net shown only when every live
// provider is down (English is acceptable here; it is clearly badged offline).
type ScriptSet = { en: string; ta: string; tanglish: string; hi?: string; hinglish?: string };

const SCRIPTS: Record<Surface, ScriptSet> = {
  sos: {
    ...
    tanglish: "Idhu romba hard minute, but it will pass. Ne slow-a oru deep breath edu… in… out. This craving is a wave — it rises and settles on its own. Konjam water kudi. You've come further than you think.",
  },
  caregiver: {
  ...
  checkin: {
    ...
    tanglish: "Check-in panninadhukku nandri — showing up itself counts. Whatever today held, you were honest, and that matters. Ne konjam gentle-a iru tonight. One small thing: moonu slow breath edu before your next thing.",
  },
  slip: {
    ...
    tanglish: "Idhu ne build panninadha erase pannaadhu — the days you've lived still count, all of them. Oru slip is one moment, not who you are. Ne konjam kind-a iru to yourself now. One small thing: oru person kitta reach out pannu.",
  },
  swap: {
    ...
    tanglish: "Indha money you'd spend today could be something you actually want. It adds up faster than it feels. Ne save panradhu oru second imagine pannu. One small thing: today's amount-a somewhere you can see it grow-la move pannu.",
  },
  okay: {
    en: "You rode that out. I'm proud of you.",
    ta: "நீங்க அதைக் கடந்துட்டீங்க. உங்களை நினைச்சு பெருமைப்படுறேன்.",
    tanglish: "Ne adha cross panniten. I'm proud of you.",
  },
  onboarding: {
    ...
    tanglish: "I've got you — inime ne idha alone carry panna vendaam. Whenever it gets heavy, naan one press away iruppen.",
  },
};

export function offlineScript(surface: Surface, language: Language): string {
  const s = SCRIPTS[surface];
  switch (language) {
    case "ta":
      return s.ta;
    case "tanglish":
